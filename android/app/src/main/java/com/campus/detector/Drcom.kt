 ...
 *
 * 关键点：必须 Proxy.NO_PROXY 直连——如果吃系统代理，认证请求会被代理劫持，
...
object Drcom {
    ...
    private fun httpGet(urlStr: String, referer: String, timeoutMs: Int): Pair<Int, String> {
    ...
    fun checkStatus(cfg: JSONObject, timeoutMs: Int = 4000): AuthStatus {
        ...
        return try {
            ...
            val (code, body) = httpGet(url, portalUrl, timeoutMs)
            if (code != 200) {
    ...
    fun login(
        ...
        return try {
            val (code, body) = httpGet("$loginUrl?$qs", portalUrl, timeoutMs)
            if (code != 200) {
